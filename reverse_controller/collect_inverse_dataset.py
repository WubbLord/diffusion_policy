...
def sample_commands(demo_delta, scale, n_samples, rng):
    dim = demo_delta.shape[-1]
    commands = []
    ...
    while len(commands) < n_samples:
        mode = rng.random()
        if mode < 0.35:
            cmd = rng.uniform(-scale, scale).astype(np.float32)
        elif mode < 0.70:
            factor = rng.uniform(0.0, 20.0)
            noise = rng.normal(0.0, 0.10, size=dim).astype(np.float32) * scale
            cmd = factor * demo_delta + noise
        else:
            cmd = rng.normal(0.0, 0.35, size=dim).astype(np.float32) * scale
...
def collect_demo(
    ...
    with h5py.File(dataset_path, "r") as f:
        ...
        for t in range(n_steps):
            ...
            gripper = actions[t, gripper_action_indices]
    ...
    actual_delta = np.zeros_like(commands, dtype=np.float32)
...
def main():
    ...
    metadata = {
        ...
        "format": {
            ...
            "desired_delta": "actual joint delta produced by command probe",
            "command": "physical JOINT_POSITION delta command in radians",
