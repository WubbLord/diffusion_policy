...
def main(
    ...
    cfg = payload["cfg"]
    adapter_payload = torch.load(open(adapter_checkpoint, "rb"), map_location="cpu")
    adapter_meta = adapter_payload.get("dataset_metadata", {})

    with open_dict(cfg.task.env_runner):
        cfg.task.env_runner._target_ = (
            "diffusion_policy.env_runner.robomimic_joint_osc_adapter_runner."
            "RobomimicJointOSCAdapterRunner")
        cfg.task.env_runner.adapter_checkpoint = adapter_checkpoint
        cfg.task.env_runner.adapter_device = adapter_device
        cfg.task.env_runner.adapter_obs_keys = adapter_meta.get("obs_keys", list(cfg.task.obs_keys))
        cfg.task.env_runner.command_scale = 1.0
        cfg.task.env_runner.use_policy_gripper = True
        for key, value in {
            "n_test": n_test,
